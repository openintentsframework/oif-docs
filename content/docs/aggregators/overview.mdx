...
docker run -p 3000:3000 -v ./config.json:/config.json oif-aggregator
```
...
RUST_LOG=info ./oif-aggregator --log-format json

# Pretty format for human reading
RUST_LOG=info ./oif-aggregator --log-format pretty
```
