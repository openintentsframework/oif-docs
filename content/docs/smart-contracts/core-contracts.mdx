...
Abstract base contract that settles outputs on the destination chain for both native and ERC20 tokens, while serving as an attester for oracle verification. Concrete contracts must implement `_resolveOutput(...)`.
