...
**Pluggable Implementations**: The `DeliveryInterface` trait allows different blockchain implementations to be used interchangeably through a registry pattern.
...
The `DeliveryInterface` trait provides an abstraction layer for different blockchain implementations. Currently, only EVM is implemented via `AlloyDelivery`. The `DeliveryService` routes operations to the appropriate implementation based on `chain_id`.
...
```rust
// All EVM chains share one AlloyDelivery instance
HashMap {
