...
sol! {
    struct StandardOrder {
        ...
        uint32 fillDeadline;
        address inputOracle;
        uint256[2][] inputs;      // [token, amount] pairs
...
Parse log into Intent:

```rust
fn parse_open_event(log: &Log) -> Result<Intent, DiscoveryError> {
    // Decode event using Alloy
    let open_event = Open::decode_log(&log)?;
    ...
    if order.outputs.is_empty() {
        return Err(ValidationError("Order must have at least one output"));
    }

    // Convert to internal format
    let order_data = Eip7683OrderData {
        user: hex::encode(order.user),
        nonce: order.nonce,
        origin_chain_id: order.originChainId,
        expires: order.expires,
        fill_deadline: order.fillDeadline,
        inputs: order.inputs,
        outputs: order.outputs.iter().map(convert_output).collect(),
        order_id: order_id.0,
        raw_order_data: Some(hex::encode(order.abi_encode())),
        lock_type: Some(LockType::Permit2Escrow),
        signature: None,  // On-chain orders don't need signatures
        sponsor: None,
    };
...
pub struct IntentMetadata {
    pub requires_auction: bool,          // Whether order requires auction process
    pub exclusive_until: Option<u64>,    // Exclusive solver until this timestamp
