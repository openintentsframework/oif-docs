...
Solvers are the execution layer of OIF. They are autonomous services that compete to fulfill user intents efficiently and profitably. When a user expresses an intent (like "I want to swap 100 USDC on Base for ETH on Arbitrum"), solvers discover these intents, and handle all the cross-chain complexity on the user's behalf.
...
- **Customize Strategy**: Implement your own execution strategies and profitability models
...
- **Flexible Authorization**: Multiple authorization mechanisms for different use cases:
  ...
  - **TheCompact**: Resource lock mechanism that allows users to allocate tokens for specific purposes without transferring custody
...
Help new Layer 2 networks attract liquidity and users by enabling seamless cross-chain transfers. Solvers can provide instant liquidity for users moving assets to new L2s, reducing friction and accelerating network adoption.
...
- **[How It Works](how-it-works.md)**: Learn about the solver's execution pipeline and core concepts
- **[Configuration Guide](configuration.md)**: Advanced setup options and customization
