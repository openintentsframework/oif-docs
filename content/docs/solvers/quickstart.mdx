...
3. **Learn Configuration**: Review the [Configuration Guide](configuration.md) for advanced setup options
4. **Understand the API**: Read the [API Reference](api-reference.md) to integrate with your applications
5. **Try Different Settlement Types**:
   - Local: Test with both `--settlement escrow` and `--settlement compact`
   - Testnet: Test with `--settlement hyperlane` for real cross-chain messaging
